...
for i =num
    rawdata=[];
    StrthRsp=[];
    RMS = [0 0.07 0.14 0.21];
    cycle_num = cycle_num+1;
    
    load([subjectid '_paramatrix']);
    
    paramatrix = paramatrix(paramatrix(:,8)==RMS(i),:);
    yfit_mat= {};
    output_mat = [];
    TrialCount_mat = {};
...
mat_subject = {'R2','PSE','x','y','trialcount','accuracy';...
    ...
    if size(num,2)==4
        subplot(2,3,5);
        plot(RMS,PSE_mat(1,:),'LineWidth',2,'Color',[1 0 0]);
        hold on 
        subplot(2,3,5);
        plot(RMS,PSE_mat(2,:),'LineWidth',2,'Color',[0 1 0]);
        hold on
        subplot(2,3,5);
        plot(RMS,PSE_mat(3,:),'LineWidth',2,'Color',[0 0 1]);
        hold on 
    elseif size(num,2)==3
        subplot(2,2,4);
        plot(RMS,PSE_mat(1,:),'LineWidth',2,'Color',[1 0 0]);
...
% 无偏拟合命令
% 去除矩阵中的nan元素
StimStrength = StimStrength( ~ isnan(StimStrength));
RespAccuracy = RespAccuracy( ~ isnan(RespAccuracy));
TrialCount = TrialCount( ~ isnan(TrialCount));
