function equality(subjectid,start,over,beep,red,green)
%%  equality +stair (cRef = 0.6)
% last edit by PWN 2020/9/17
...
if  round(frameRate)~=100                                                  % 确保刷新频率是100hz程序才能运行
    quit
end
...
% % greencircleDura 用于消除橙色基座的绿色后效
% greencircleDura = 1000;
% greencircleFrames = round(greencircleDura/frameDura);
...
boundary = 0.5;%光栅模糊边界是0.5度

cAdaption = 0.14;%注意此处的cAdaption已经恢复成RMS对比度2020/9/17
...
for index = start:over
    ...
    Ifisoluminance = paramatrix(index,8);
    %     cAdaption = 1;
    ...
    phase = rand(1)*2*pi;
    MatrixTest = MakeLumGrating(sizeGrating,angleTest,cTest,red,green,blue,sf,phase,boundary);
    test = Screen('MakeTexture',w, MatrixTest);
    MatrixRef = MakeLumGrating(sizeGrating,angleRef,cRef,red,green,blue,sf,phase,boundary);
    ...
    [MatrixAdaptionGrating,AdaptionGrating_luminance,AdaptionGrating_Color_contrast] =  ...
        TextureColorNoiseMask(Adaption_sizeGrating,cAdaption, ...
        Adaption_angle,Adaption_angle_sigma,Adaption_sf, ...
        Adaption_sf_sigma,Adaption_boundary,red,green,Ifisoluminance);
    paramatrix(index,16) = AdaptionGrating_luminance;%记录noise亮度，在后面的分析中可能会用上
    paramatrix(index,17) = AdaptionGrating_Color_contrast;%记录Michelson对比度，在后面的分析中可能会用上
...
%     [MatrixGreenCircle] = TextureGreenCircle(Adaption_sizeGrating,Adaption_boundary,green);
%     Left_GreenCircle = Screen('MakeTexture',w, MatrixGreenCircle);
%     Right_GreenCircle = Screen('MakeTexture',w, MatrixGreenCircle);
%     GreenCircleTexture = [Left_GreenCircle Right_GreenCircle];
%     desGreenCircle = reshape([Adaption_desCenter(:,:,1) Adaption_desCenter(:,:,2)],4,2);
...
%     for r = 1: greencircleFrames
%         Screen('DrawTextures', w,GreenCircleTexture,[],desGreenCircle); % gratings
%         %             Screen('FillRect', w, black, desFix);
%         %             Screen('DrawLines',w,desFix,linethickness,black,[],1);
%         Screen('DrawDots',w,desFix,sizeFix,black,[],1);
%         start_time = Screen('Flip',w);
%     end
    ...
    %% save
    save(filename,'paramatrix','red','green');
    ...
    Screen('close',Adaption_gratingTexture);
%     Screen('close',GreenCircleTexture);
    % close screen
